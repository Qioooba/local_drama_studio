"""Frozen-snapshot Worker handler for managed llama.cpp structured text tasks."""
...
_HANDLER_CODE = "llama.text.v2"
_HANDLER_VERSION = "v1"
_ADAPTER_CODE = "llama.chat.v1"
_CAPABILITIES = frozenset({"LLM_STORY_PARSE", "LLM_EPISODE_PLAN", "LLM_STORYBOARD", "LLM_PROMPT_REWRITE"})


class LlamaCppStructuredClient(Protocol):
    def chat_json(
        self,
        system: str,
        user: str,
...
def make_llama_cpp_text_handler(
    settings: Settings,
    *,
    client_factory: Callable[[str], LlamaCppStructuredClient] | None = None,
    ...
    def execute(snapshot: WorkerExecutionSnapshot, output_root: Path) -> tuple[str, str]:
        ...
        try:
            ...
            raise DomainRuleError("MP_LLAMA_TEXT_EXECUTION_FAILED", "托管 llama.cpp V2 文本执行失败。") from error
        target = output_root / "llama" / "result.json"
        artifact = {"schema": "localdramastudio.llama-text-result.v1", "capability_code": snapshot.capability_code, "result": result}
        ...
        return "LLAMA_TEXT_RESULT", relative
...
def _assert_snapshot(snapshot: WorkerExecutionSnapshot, settings: Settings) -> str:
    if snapshot.capability_code not in _CAPABILITIES or snapshot.adapter_code != _ADAPTER_CODE:
        raise DomainRuleError("MP_LLAMA_TEXT_SNAPSHOT_MISMATCH", "冻结快照不属于受控托管 llama.cpp V2 文本 Handler。")
    if str(snapshot.network_policy.get("mode") or "").upper() != "LOCAL_ONLY":
        raise DomainRuleError("MP_LLAMA_TEXT_NETWORK_POLICY_INVALID", "托管 llama.cpp V2 文本 Handler 只允许 LOCAL_ONLY 网络策略。")
    try:
        verify_managed_llama_runtime_configuration(
            settings,
            json.dumps(dict(snapshot.runtime_configuration), ensure_ascii=False, sort_keys=True, separators=(",", ":")),
        )
    except DomainRuleError as error:
        if error.code == "MP_RUNTIME_CONFIGURATION_STALE":
            raise DomainRuleError("MP_LLAMA_TEXT_RUNTIME_STALE", "冻结托管 Runtime 与当前服务身份配置不一致。") from error
        raise
    if len(snapshot.model_bindings) != 1:
        raise DomainRuleError("MP_LLAMA_TEXT_MODEL_BINDING_INVALID", "托管 llama.cpp V2 文本任务必须绑定一个已验证模型。")
    model = snapshot.model_bindings[0].get("native_locator")
    if not isinstance(model, str) or not model.strip():
        raise DomainRuleError("MP_LLAMA_TEXT_MODEL_BINDING_INVALID", "冻结模型绑定缺少 llama-server 模型别名。")
    return Path(model.strip()).stem
...
def _prompts(snapshot: WorkerExecutionSnapshot) -> tuple[str, str]:
    ...
    if not all(isinstance(value, str) and value.strip() and len(value) <= 8192 for value in (system, user)):
        raise DomainRuleError("MP_LLAMA_TEXT_INPUT_INVALID", "托管 llama.cpp V2 文本任务需要 1—8192 字符的 system_prompt 与 user_prompt。")
...
def _schema(snapshot: WorkerExecutionSnapshot) -> dict[str, object] | None:
    ...
    if not isinstance(value, dict):
        raise DomainRuleError("MP_LLAMA_TEXT_BINDING_INVALID", "冻结的 json_schema 必须是 JSON 对象。")
    return {str(key): item for key, item in value.items()}


def llama_cpp_text_handler_identity() -> tuple[str, str]:
