...
def _context(workspace, database):
    ...
    revision = service.create_shot_revision(
        str(first["id"]),
...
def test_semantic_reorder_and_split_preserve_lineage(workspace, database) -> None:
    ...
    with TestClient(create_app(workspace)) as client:
        ...
        reordered = client.post(f"/api/v1/episodes/{episode_id}/shot-edit:commit", json={
            **reorder_payload, "expected_plan_hash": reorder_plan["plan_hash"],
        })
        ...
        split_payload = {
            "ordering_token": context["ordering_token"], "reorder": None,
            "splits": [{
                "shot_id": first["id"], "expected_revision": first_snapshot["revision"],
                "first_code": "SHOT_001-A", "second_code": "SHOT_001-B", "first_duration_ms": 1500,
            }],
        }
        ...
        assert split_plan["effects"] == {
            "timeline": "MARK_STALE", "selected_results": "NOT_COPIED", "asset_bindings": "COPY",
        }
        split = client.post(f"/api/v1/episodes/{episode_id}/shot-edit:commit", json={
            **split_payload, "expected_plan_hash": split_plan["plan_hash"],
        })
    ...
    with database.connect() as connection:
        ...
        child_rows = connection.execute(
            "SELECT * FROM shots WHERE source_shot_id=? ORDER BY code", (first["id"],),
        ).fetchall()
        assert [row["target_duration_ms"] for row in child_rows] == [1500, 2500]
        for row in child_rows:
            fields = json.loads(connection.execute(
                "SELECT fields_json FROM shot_revisions WHERE id=?", (row["current_revision_id"],),
            ).fetchone()[0])
            ...
            assert connection.execute(
                "SELECT COUNT(*) FROM shot_asset_bindings WHERE shot_id=? AND asset_id=?", (row["id"], asset_id),
            ).fetchone()[0] == 1
        assert connection.execute(
        ...
        actions = {row[0] for row in connection.execute(
            "SELECT action FROM audit_events WHERE subject_id IN (?,?)", (episode_id, first["id"]),
        )}
...
def test_shot_edit_rejects_stale_ordering_token(workspace, database) -> None:
    ...
    with TestClient(create_app(workspace)) as client:
        plan = client.post(f"/api/v1/episodes/{episode['id']}/shot-edit:plan", json={
            "ordering_token": "0" * 64,
