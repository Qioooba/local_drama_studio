...
def test_shot_ready_requires_explicit_fields(workspace, database) -> None:
    ...
    shot_studio_command_service(database).save_draft_revision(shot["id"], {"subject_action": "walk"}, freeze=True)
    with pytest.raises(DomainRuleError) as error:
        shot_studio_command_service(database).mark_ready_shot(shot["id"])
