...
try {
  for (const viewport of viewports) {
    ...
    page.on("request", (request) => { const url = new URL(request.url()); if (!["127.0.0.1", "localhost"].includes(url.hostname)) publicRequests.push(request.url()); if (/project-packages|packages:(export|dry-run)/.test(url.pathname)) packageMutations.push(request.url()); if (/\/media(-versions)?\//.test(url.pathname) && url.searchParams.get("size") !== "small") originalMediaRequests.push(request.url()); });
    ...
    const policyVisible = await page.getByText(/导入只读取固定本地 inbox/).isVisible();
    const inboxVisible = await section.getByLabel("Inbox 文件名").isVisible();
    const pathBoundaryVisible = await section.getByText(/这里只填写文件名，不接受任意路径/).isVisible();
    const buttonHeight = await section.getByRole("button", { name: "导出并 dry-run" }).evaluate((item) => item.getBoundingClientRect().height);
    const inboxHeight = await section.getByLabel("Inbox 文件名").evaluate((item) => item.getBoundingClientRect().height);
    ...
    results.push({ viewport, visible, policyVisible, inboxVisible, pathBoundaryVisible, buttonHeight, inboxHeight, geometry, packageMutations, consoleProblems, pageErrors, failedResponses, publicRequests, originalMediaRequests, mutationClicked: false });
    await page.close();
  }
