...
const categoryLabel = (category: QcCategory) => CATEGORIES.find(([id]) => id === category)?.[1] ?? "其他检查";
const stageLabel = (stage: QcStage) => STAGES.find(([id]) => id === stage)?.[1] ?? "其他阶段";
const STAGE_RECOMMENDED_THRESHOLDS: Record<QcStage, Partial<Record<QcCategory, number>>> = {
  IMAGE: { VISUAL: .8, FACE: .88, IDENTITY: .88, COMPOSITION: .78, TECHNICAL: .9 },
  VIDEO: { VISUAL: .8, FACE: .85, IDENTITY: .88, COMPOSITION: .78, MOTION: .82, CONTINUITY: .85, TECHNICAL: .9 },
  AUDIO: { AUDIO: .88, TECHNICAL: .9 },
  CONTINUITY: { IDENTITY: .88, CONTINUITY: .9, MOTION: .8 },
  DELIVERY: { VISUAL: .8, AUDIO: .85, TECHNICAL: .95 },
};
...
export function QcPolicyManager({ projectId, initialEpisodeId = "", initialShotId = "" }: { projectId: string; initialEpisodeId?: string; initialShotId?: string }) {
  ...
  const save = useMutation({
    ...
    onSuccess: async (saved) => { setNotice(`已创建不可变的第 ${saved.version_no} 版，规则记录 ${saved.revision}`); await cache.invalidateQueries({ queryKey: ["qc-policy"] }); },
    onError: (error) => setNotice(error instanceof QcPolicyApiError && error.status === 409 ? `版本冲突：${error.message}。刷新后再保存。` : `保存失败：${message(error)}`),
  });
  const recommendedThreshold = (category: QcCategory) => resolution.data?.policy?.thresholds?.[category] ?? STAGE_RECOMMENDED_THRESHOLDS[stage][category] ?? .8;
  const toggleThreshold = (category: QcCategory, enabled: boolean) => setThresholds((old) => { const next = { ...old }; if (enabled) next[category] = recommendedThreshold(category); else delete next[category]; return next; });
  ...
  const adjustThreshold = (category: QcCategory, direction: number) => setThresholds((old) => ({ ...old, [category]: Math.max(0, Math.min(1, Number(((old[category] ?? recommendedThreshold(category)) + direction * .05).toFixed(2)))) }));
  ...
  if (initialPending) return <section className="panel" role="status" aria-live="polite"><p className="empty-state">正在读取自动质检规则、继承范围与当前生效版本…</p></section>;
  if (initialErrors.length > 0) return <section className="panel" role="alert"><div className="panel-heading"><div><p className="eyebrow">读取失败</p><h3>自动质检规则暂时无法打开</h3></div></div><p className="muted">已保留页面上下文，没有写入任何策略。{initialErrors.map(message).join("；")}</p><button type="button" className="secondary" onClick={() => void Promise.all([seasons.refetch(), policies.refetch(), resolution.refetch()])}>重新读取</button></section>;
  ...
  return <div className="qc-policy-manager">
    ...
    <div className="qc-workspace">
      <section className="panel qc-resolution" aria-labelledby="qc-resolution-title"><div className="panel-heading"><div><p className="eyebrow">当前生效策略</p><h3 id="qc-resolution-title">{stageLabel(stage)}</h3></div><span className={`status-pill ${resolution.data ? "state-active" : "neutral"}`}>{resolution.data ? OWNER_LABEL[resolution.data.source] : "未配置"}</span></div>
        <ol className="qc-inheritance">{hierarchy.map(({ type, id }) => { const item = id ? findPolicy(policies.data ?? [], type, id, stage) : undefined; return <li className={resolution.data?.source === type ? "effective" : ""} key={type}><span>{type === "PROJECT" ? "1" : type === "EPISODE" ? "2" : "3"}</span><div><strong>{OWNER_LABEL[type]}</strong><small>{!id ? "未选择" : item ? `第 ${item.version_no} 版 · 规则记录 ${item.revision}` : "继承上级"}</small></div></li>; })}</ol>
        {resolution.data ? <dl className="qc-resolution-facts"><div><dt>生效版本</dt><dd>第 {resolution.data.version_no} 版</dd></div><div><dt>自动重试上限</dt><dd>{resolution.data.max_auto_rerolls} 次</dd></div><div><dt>允许自动重试</dt><dd>{resolution.data.auto_reroll_categories.map(categoryLabel).join("、") || "无，失败后转人工处理"}</dd></div></dl> : <p className="empty-state">该范围没有可继承策略，生产前应先配置项目默认。</p>}
      </section>
      <form className="panel qc-editor" onSubmit={(e) => { e.preventDefault(); save.mutate(); }}><div className="panel-heading"><div><p className="eyebrow">版本化编辑</p><h3>{OWNER_LABEL[ownerType]} · {stageLabel(stage)}</h3></div><span className="status-pill neutral">{current ? `规则记录 ${current.revision}` : "新策略"}</span></div>
        ...
        <fieldset className="qc-thresholds"><legend>检查类别与通过阈值</legend>{CATEGORIES.map(([id, label]) => { const active = thresholds[id] !== undefined; const recommended = recommendedThreshold(id); return <div key={id}><label><input type="checkbox" checked={active} onChange={(e) => toggleThreshold(id, e.target.checked)} />{label}<small>建议 {Math.round(recommended * 100)}%（{resolution.data?.policy?.thresholds?.[id] !== undefined ? "继承生效策略" : `${stageLabel(stage)}阶段基线`}）</small></label><label className="threshold-value"><span className="sr-only">{label}阈值</span><input type="number" min="0" max="1" step="0.05" value={thresholds[id] ?? recommended} disabled={!active} onKeyDown={(e) => { if (["ArrowUp", "ArrowRight"].includes(e.key)) { e.preventDefault(); adjustThreshold(id, 1); } else if (["ArrowDown", "ArrowLeft"].includes(e.key)) { e.preventDefault(); adjustThreshold(id, -1); } }} onChange={(e) => setThresholds((old) => ({ ...old, [id]: Math.max(0, Math.min(1, Number(e.target.value))) }))} /><output>{Math.round((thresholds[id] ?? recommended) * 100)}%</output></label></div>; })}</fieldset>
