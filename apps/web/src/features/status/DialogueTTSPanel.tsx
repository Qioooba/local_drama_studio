...
import { selectTTSCandidate, type DialogueLine, type VoiceProfileVersion } from "../../generated/api";
import { DialogueGovernanceActions } from "./DialogueGovernanceActions";

export function DialogueTTSPanel({ lines, voices, projectId, episodeId, onChanged }: { lines: DialogueLine[]; voices: VoiceProfileVersion[]; projectId?: string; episodeId?: string; onChanged?: () => void }) {
  ...
  return <section className="panel dialogue-tts-panel" aria-labelledby="dialogue-tts-title">
    ...
    {projectId && episodeId && onChanged && <DialogueGovernanceActions projectId={projectId} episodeId={episodeId} lines={lines} voices={voices} onChanged={onChanged} />}
  </section>;
}
