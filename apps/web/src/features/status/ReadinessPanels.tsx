...
export function ProjectConfigurationSnapshot({ configuration, projectId, onChanged }: { configuration: ProjectConfiguration; projectId?: string; onChanged?: () => void }) {
  ...
  return <section className="panel configuration-snapshot" aria-labelledby="configuration-snapshot-title">
    <div className="panel-heading"><div><p className="eyebrow">G7 项目配置</p><h3 id="configuration-snapshot-title">项目配置快照与切换影响</h3></div><span className="status-pill">只读 · 仅本地</span></div>
...
export function AdapterContractsPanel({ registry }: { registry?: AdapterRegistry }) {
  return <section className="panel adapter-contracts" aria-labelledby="adapter-contracts-title">
    <div className="panel-heading"><div><p className="eyebrow">G7 适配器 SDK</p><h3 id="adapter-contracts-title">本地适配器契约</h3></div><span className="status-pill">静态检查 · 无运行时接触</span></div>
    ...
    <div className="configuration-table" role="table" aria-label="本地适配器契约">
      ...
      {(registry?.contracts ?? []).map((item) => <div className="configuration-row" role="row" key={item.code}><span title={item.title}>{item.title}</span><span title={item.transport}>{item.transport}</span><span className="status-pill" title={item.status}>{item.status}</span><span title={(item.capabilities ?? []).join(" · ")}>{(item.capabilities ?? []).slice(0, 3).join(" · ")}{(item.capabilities ?? []).length > 3 ? ` · +${(item.capabilities ?? []).length - 3}` : ""}</span></div>)}
...
export function ModelCompatibilityPanel({ snapshot, projectId, onEvidenceImported }: { snapshot: ModelCompatibilitySnapshot; projectId?: string; onEvidenceImported?: () => void }) {
  return <section className="panel configuration-snapshot" aria-labelledby="model-compatibility-title">
    <div className="panel-heading"><div><p className="eyebrow">G7 本机模型引用</p><h3 id="model-compatibility-title">用户自带模型路径与兼容性</h3></div><span className={`status-pill${snapshot.summary.pass_count === snapshot.summary.reported_count ? "" : " neutral"}`}>{snapshot.summary.pass_count}/{snapshot.summary.reported_count} COMPATIBLE</span></div>
    <div className="configuration-grid">
    ...
    <div className="configuration-table" role="table" aria-label="模型兼容性证据">
      ...
      {snapshot.reports.slice(0, 8).map((item) => <div className="configuration-row" role="row" key={item.artifact_id}><span title={`${item.code} · ${item.kind}`}>{item.code}<small>{item.kind}</small></span><span title={`${item.report_sha256 ? `${item.report_sha256.slice(0, 12)}…` : "未报告"} · ${String(item.quantization.status ?? "UNKNOWN")} · 能力：${String((item.quantization.capability as { status?: string } | undefined)?.status ?? "未请求")}`}>{item.report_sha256 ? `${item.report_sha256.slice(0, 12)}…` : "未报告"} · {String(item.quantization.status ?? "UNKNOWN")}<small>能力：{String((item.quantization.capability as { status?: string } | undefined)?.status ?? "未请求")}</small></span><span title={String(item.has_license_evidence ? item.license_path_rel : "用户未声明 · 自行负责")}>{item.has_license_evidence ? item.license_path_rel : "用户未声明 · 自行负责"}</span><span className={`status-pill${item.report_status === "PASS" ? "" : " neutral"}`} title={String(item.blockers.join(" · ") || item.report_status || "未报告")}>{item.report_status ?? "未报告"}{item.blockers.length > 0 && <small>{item.blockers.join(" · ")}</small>}</span></div>)}
    </div>
...
export function TimelineStatusPanel({ status }: { status: TimelineStatus }) {
  ...
  return <section className="panel timeline-status-panel" aria-labelledby="timeline-status-title">
    <div className="panel-heading"><div><p className="eyebrow">G8 时间线 / 交付</p><h3 id="timeline-status-title">时间线与交付状态</h3></div><span className="status-pill neutral">只读观测</span></div>
    ...
    <div className="configuration-grid capacity-grid">
      <div className="configuration-card"><small>时间线修订</small><strong>{status.timeline.revision_count}</strong><span>{latestTimeline ? `最新 v${String(latestTimeline.revision_no)}` : "暂无真实 revision"}</span></div>
    ...
    <div className="canvas-status"><span>含用户授权记录：{status.audio.verified_local_count}</span><span>字幕文本权威：{latestSubtitle?.authority_status === "VERIFIED_SCRIPT" ? "SCRIPT" : "未验证"}</span><span>ASR：{latestSubtitle?.asr_alignment_only ? "仅时间对齐" : "未参与"}</span><span>已验证渲染：{status.renders.verified_count}</span><span>已验证交付：{status.delivery.verified_count}</span></div>
  </section>;
}
...
export function G8ReadinessPanel({ readiness }: { readiness: G8Readiness }) {
  return <section className="panel gate-readiness" aria-labelledby="g8-readiness-title">
    <div className="panel-heading"><div><p className="eyebrow">G8 正式退出就绪</p><h3 id="g8-readiness-title">整集音频、字幕、时间线与交付门禁</h3></div><span className={`status-pill${readiness.status === "PASS" ? "" : " neutral"}`}>{readiness.status}</span></div>
    <p className="muted">只读检查蓝图 09 的正式退出条件；不会创建素材、启动 ComfyUI 或自动替代整集人工批准。当前集：{readiness.episode.code} · {readiness.episode.title}</p>
    <ol className="gate-checks">{readiness.checks.map((check) => <li className={check.passed ? "passed" : "blocked"} key={check.code}><GateStatusIcon passed={check.passed} /><div className="gate-check-body"><strong>{g8CheckLabels[check.code] ?? check.code}</strong>{check.count !== undefined && <small>{check.count} 项真实证据</small>}<small>{check.detail}</small></div></li>)}</ol>
    ...
    <div className="canvas-status"><span>timeline {readiness.evidence.timeline_revision_id ? "已锁定" : "缺失"}</span><span>render {readiness.evidence.render_id ? "已记录" : "缺失"}</span><span>delivery {readiness.evidence.delivery_id ? "已记录" : "缺失"}</span></div>
  </section>;
}
...
export function G9ReadinessPanel({ readiness }: { readiness: G9Readiness }) {
  return <section className="panel gate-readiness" aria-labelledby="g9-readiness-title">
    <div className="panel-heading"><div><p className="eyebrow">G9 正式退出就绪</p><h3 id="g9-readiness-title">业务画布与生产效率门禁</h3></div><span className={`status-pill${readiness.status === "PASS" ? "" : " neutral"}`}>{readiness.status}</span></div>
    <p className="muted">只读区分生产图事实与规模 fixture 证据；不会创建镜头、布局、执行计划或 Job。当前集：{readiness.episode.code} · {readiness.episode.title}</p>
    <ol className="gate-checks">{readiness.checks.map((check) => <li className={check.passed ? "passed" : "blocked"} key={check.code}><GateStatusIcon passed={check.passed} /><div className="gate-check-body"><strong>{g9CheckLabels[check.code] ?? check.code}</strong>{check.count !== undefined && <small>{check.count} 项真实观测</small>}<small>{check.detail}</small></div></li>)}</ol>
    ...
    <div className="canvas-status"><span>生产镜头：{readiness.evidence.production_total_shots}</span><span>可见节点：{readiness.evidence.production_visible_nodes}</span><span>layout：{readiness.evidence.persisted_layout_count}</span><span>preflight：{readiness.evidence.persisted_preflight_count}</span></div>
  </section>;
}
...
export function DiagnosticPanel({ run }: { run: { status: string; checks: Array<{ code: string; status: string; observed: Record<string, unknown> }> } | null }) {
  ...
  return <div className="diagnostic-grid"><div className="diagnostic-status"><span>整体状态</span><strong className={statusClass(run.status)}>{run.status}</strong></div>{run.checks.map((check, index) => <div className="diagnostic-row" key={`${check.code}-${index}`}><span>{check.code}</span><strong className={statusClass(check.status)}>{check.status}</strong></div>)}</div>;
}
