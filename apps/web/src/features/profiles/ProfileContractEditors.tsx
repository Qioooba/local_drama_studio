...
export function ProfileContractEditors(props: EditorProps) {
  ...
  const applyTemplate = () => {
    ...
    write(patchNested(parameters.data, "seed", { determinism: "EXPLICIT", required: true }), props.onParameterChange);
  ...
  return <div className="profile-contract-structured">
    <div className="profile-contract-fields">
      <label>本地连接方式<select value={String(input.data.transport ?? "")} onChange={(event) => write({ ...input.data, transport: event.target.value }, props.onInputChange)}><option value="">请选择</option><option value="LOOPBACK_HTTP">本机 HTTP 接口</option><option value="LOCAL_PROCESS">本机进程</option><option value="LOCAL_CLI">本机命令行程序</option></select><small>不允许连接远程服务。</small></label>
