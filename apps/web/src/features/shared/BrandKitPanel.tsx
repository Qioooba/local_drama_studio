...
export function BrandKitPanel({ projectId }: { projectId: string }) {
  ...
  return <section className="panel brand-kit-panel" aria-labelledby="brand-kit-title"><div className="panel-heading"><div><p className="eyebrow">FR-PST-003 · BRAND / WATERMARK / COMPLIANCE</p><h3 id="brand-kit-title">本地品牌与合规版本</h3></div><span className="status-pill neutral">LOCAL VERSIONED</span></div><p className="muted">BrandKit、WatermarkProfile、CompliancePolicy 都是不可变版本。交付会读取当前 ACTIVE 版本并写入 manifest；机器预检只负责规则与文件完整性，人工/平台审核仍保持 PENDING。</p><div className="field-grid"><label>BrandKit 代码<input value={code} onChange={(event) => setCode(event.target.value)} /></label><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} /></label></div><label>BrandKit Token JSON<textarea value={tokens} onChange={(event) => setTokens(event.target.value)} spellCheck={false} /></label><button className="primary-action" type="button" onClick={() => void create()} disabled={busy}>{busy ? "发布中…" : "发布 BrandKit 新版本"}</button><div className="field-grid"><label>水印文字<input value={watermarkText} onChange={(event) => setWatermarkText(event.target.value)} /></label><label>位置<select value={watermarkPosition} onChange={(event) => setWatermarkPosition(event.target.value)}><option value="TOP_LEFT">左上</option><option value="TOP_RIGHT">右上</option><option value="BOTTOM_LEFT">左下</option><option value="BOTTOM_RIGHT">右下</option><option value="CENTER">居中</option></select></label></div><button className="secondary" type="button" onClick={() => void publishWatermark()} disabled={busy}>发布 WatermarkProfile 新版本</button><div className="field-grid"><label>最长时长（毫秒）<input type="number" min="1" value={maxDuration} onChange={(event) => setMaxDuration(event.target.value)} /></label><span className="review-meta">新交付要求水印，机器预检通过后仍需人工/平台复核</span></div><button className="secondary" type="button" onClick={() => void publishCompliance()} disabled={busy}>发布 CompliancePolicy 新版本</button><div className="review-meta"><span>Watermark ACTIVE：{controls.watermark_profiles.filter((item) => item.status === "ACTIVE").length}</span><span>Compliance ACTIVE：{controls.compliance_policies.filter((item) => item.status === "ACTIVE").length}</span></div>{message && <p className="review-success" role="status">{message}</p>}{error && <p className="inline-error" role="alert">{error}</p>}</section>;
}
