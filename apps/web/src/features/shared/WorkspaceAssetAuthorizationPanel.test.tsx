...
describe("WorkspaceAssetAuthorizationPanel labels", () => {
  it("uses shot code as the primary label and keeps the version id in advanced details", () => {
    render(<WorkspaceAssetAuthorizationPanel projectId="project-1" items={[{
      ...
      episode_code: "EP03", shot_code: "S12", media_kind: "VIDEO", stage: "PROXY", decision: null, is_stale: 0, integrity_status: "VERIFIED",
    }]} />);
    ...
    expect(screen.getByText("VERIFIED · 人审待定")).toBeTruthy();
    expect(screen.getByRole("button", { name: "授权到项目" }).getAttribute("title")).toContain("不提交人工审核");
  });
});
