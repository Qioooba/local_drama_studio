...
export function WorkspaceAssetAuthorizationPanel({ projectId, items, authorizations = [], onChanged }: { projectId: string; items: ReviewInboxItem[]; authorizations?: WorkspaceAssetAuthorization[]; onChanged?: () => void }) {
  ...
  const confirmRevoke = async (mediaVersionId: string) => {
    ...
    if (!reason) return;
    setPendingId(mediaVersionId); setError(null);
    try { await revokeWorkspaceAssetAuthorization(projectId, mediaVersionId, reason); setAuthorized((current) => { const next = { ...current }; delete next[mediaVersionId]; return next; }); onChanged?.(); }
  ...
  return <section className="panel workspace-asset-panel" aria-labelledby="workspace-asset-title">
    <div className="panel-heading"><div><p className="eyebrow">FR-AST-001 · 项目授权</p><h3 id="workspace-asset-title">项目工作区资产授权</h3></div><span className="status-pill neutral">本地哈希</span></div>
    <p className="muted">只允许当前项目的 VERIFIED 媒体版本进入工作区；授权会复核本地路径、字节数和 SHA-256，不复制模型或媒体。</p>
    {candidates.length === 0 ? <p className="empty-state">暂无可授权的审核媒体候选。</p> : <div className="configuration-table" role="table" aria-label="项目工作区资产候选"><div className="configuration-row configuration-header" role="row"><strong>所属镜头</strong><strong>类型 / 阶段</strong><strong>审核状态</strong><strong>操作</strong></div>{candidates.map((item) => { const existing = authorizationByMedia.get(item.media_version_id); const isAuthorized = existing?.authorization_status === "AUTHORIZED" || Boolean(authorized[item.media_version_id]); return <div className="configuration-row" role="row" key={item.media_version_id}><span><strong>{item.shot_code ?? item.episode_code ?? "项目级媒体"}</strong><details><summary>高级：版本标识</summary><code>{item.media_version_id}</code></details></span><span>{item.media_kind} · {item.stage}</span><span>{existing?.impact.length ? `影响：${existing.impact.join("、")}` : isAuthorized ? "已授权" : item.decision ?? "待审核"}</span><span><button className="secondary" type="button" onClick={() => void authorize(item.media_version_id)} disabled={pendingId !== null || isAuthorized}>{pendingId === item.media_version_id ? "校验中…" : isAuthorized ? "已授权" : "授权到项目"}</button>{isAuthorized && <button className="secondary" type="button" onClick={() => { setRevokingId(item.media_version_id); setRevokeReason(""); setError(null); }} disabled={pendingId !== null}>撤回</button>}</span>{revokingId === item.media_version_id && <div className="revoke-inline" role="dialog" aria-label="填写撤回授权原因"><input autoFocus aria-label="撤回授权原因" value={revokeReason} onChange={(event) => setRevokeReason(event.target.value)} placeholder="撤回授权原因（必填）" /><button className="primary-action" type="button" onClick={() => void confirmRevoke(item.media_version_id)} disabled={!revokeReason.trim() || pendingId !== null}>{pendingId === item.media_version_id ? "撤回中…" : "确认撤回"}</button><button className="secondary" type="button" onClick={() => { setRevokingId(null); setRevokeReason(""); }} disabled={pendingId !== null}>取消</button></div>}</div>; })}</div>}
