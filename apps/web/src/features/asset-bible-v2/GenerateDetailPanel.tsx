...
export function GenerateDetailPanel({ projectId, assetId, assetStatus, states, baseReferences, initialBatches, onReferencesChanged }: Props) {
  ...
  return <section className="panel multiview-panel" aria-labelledby={`detail-title-${assetId}`}>
    ...
    <div className="multiview-controls"><label>造型状态<select value={assetStateId} onChange={(event) => setAssetStateId(event.target.value)}><option value="">基础角色</option>{states.map((state) => <option value={state.id} key={state.id}>{state.label}</option>)}</select></label><label>语义 Profile<select aria-label="近景细节 Profile" value={profileVersionId} onChange={(event) => setProfileVersionId(event.target.value)}><option value="">AUTO · 项目偏好解析</option>{profiles.map((profile) => <option value={profile.version_id} key={profile.version_id}>{profile.title} · v{profile.version_no ?? "?"}</option>)}</select></label></div>
