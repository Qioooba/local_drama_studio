...
vi.mock("../../generated/api", () => ({ createFrameAnchor: vi.fn(), createShotTransitionConstraint: vi.fn() }));
...
describe("FrameBridgeControls tail extraction", () => {
  ...
  it("keeps cross-scene inheritance explicit instead of presenting it as the default", () => {
    ...
    const frameBridge: DirectorDeskResponse["current_shot"]["frame_bridge"] = { previous, current_start: null, current_end: null, next: null, compatibility: "READY", stale: false };
  ...
  it("creates a missing adjacent boundary instead of calling a non-first shot the first shot", async () => {
    const frameBridge: DirectorDeskResponse["current_shot"]["frame_bridge"] = { previous: null, current_start: null, current_end: null, next: null, compatibility: "MISSING", stale: false };
