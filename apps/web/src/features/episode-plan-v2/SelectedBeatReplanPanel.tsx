...
export function SelectedBeatReplanPanel({ projectId, episodeId }: { projectId: string; episodeId: string }) {
  ...
  return <section className="subpanel selected-beat-replan" aria-labelledby="beat-replan-title">
    <div className="panel-heading"><div><p className="eyebrow">人工审核 · Selected Beat AI Replan</p><h4 id="beat-replan-title">只重排选定 Beat</h4></div><span className="status-pill neutral">计划 → 差异 → 落地</span></div>
    <p className="muted">预览不会写入；应用只触碰所选 Beat 的镜头。冻结镜头始终保留，删除项只归档，旧 revision 与候选不会删除。</p>
    <div className="shot-group-create">
      <label>选定剧情段落<select value={groupId} onChange={(event) => { setGroupId(event.target.value); resetPreview(); }}><option value="">请选择</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.code} · {group.title} · 第 {group.revision} 次修订</option>)}</select></label>
      <label>AI 拆解草稿<select value={draftId} onChange={(event) => { setDraftId(event.target.value); resetPreview(); }}><option value="">请选择</option>{drafts.data?.map((draft) => <option key={draft.id} value={draft.id}>{draft.source_document_title} · {optionLabel(STATUS_LABELS, draft.status)}</option>)}</select></label>
      ...
      <button type="button" className="secondary" disabled={!selectedGroup || !draftId || preview.isPending} onClick={() => preview.mutate()}>{preview.isPending ? "生成中…" : "生成差异预览"}</button>
    </div>
    ...
    {plan && <div className="beat-replan-preview">
      <div className="panel-heading"><strong>{plan.group_code} · {plan.group_title}</strong><span className="status-pill neutral">范围外触碰 {plan.scope.outside_group_shots_touched}</span></div>
      ...
      <button type="button" className="primary-action" disabled={!plan.valid || apply.isPending} onClick={() => apply.mutate()}>{apply.isPending ? "应用中…" : "确认应用以上差异"}</button>
    </div>}
