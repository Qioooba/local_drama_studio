...
export function ProductionCanvasPanel({ episodeId, selectedShotId, onSelectShot }: { episodeId: string | null; selectedShotId: string | null; onSelectShot: (shotId: string) => void }) {
  ...
  return <section className="panel canvas-panel">
    <div className="panel-heading"><div><p className="eyebrow">G9 生产画布</p><h3>业务依赖 DAG · 拖动只保存布局</h3></div><div className="canvas-actions"><button type="button" className="secondary" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>{saveMutation.isPending ? "保存中…" : "保存布局"}</button><button type="button" className="secondary" onClick={() => planMutation.mutate()} disabled={!selectedNodeId || planMutation.isPending}>{planMutation.isPending ? "预检中…" : "运行节点预检"}</button></div></div>
    ...
    <div className="canvas-workspace" aria-label="业务画布"><ReactFlow nodes={visibleNodes} edges={edges} onNodesChange={onNodesChange} onNodeClick={(_, node) => { setSelectedNodeId(node.id); const match = node.id.match(/^shot:([^:]+):/); if (match) onSelectShot(match[1]); }} fitView fitViewOptions={{ padding: 0.2 }} minZoom={0.15} maxZoom={1.8} nodesConnectable={false} deleteKeyCode={null}><Background /><Controls /><MiniMap pannable zoomable nodeColor={(node) => String(node.className).includes("blocked") ? "#df9d4b" : String(node.className).includes("running") ? "#7c91ff" : "#45d3b3"} /></ReactFlow></div>
