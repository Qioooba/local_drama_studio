...
import { createEpisodeTimelineDraftV2, freezeEpisodeTimelineV2, getEpisodeEditWorkspaceV2, type EditAudioClipV2, type EditVideoClipV2 } from "../../generated/api";
...
function timeLabel(us: number) { const seconds = Math.max(0, us) / 1_000_000; const minutes = Math.floor(seconds / 60); return `${String(minutes).padStart(2, "0")}:${(seconds % 60).toFixed(1).padStart(4, "0")}`; }
...
export function EpisodeEditWorkspace({ projectId, episodeId }: { projectId: string; episodeId: string }) {
  ...
  return <div className="edit-workspace-v2">
    ...
    <section className="edit-stage-grid"><div className="edit-player-panel"><div className="edit-player">{selected?.media_version_id ? <video key={selected.media_version_id} ref={videoRef} controls playsInline preload="none" poster={`/api/v1/media-versions/${encodeURIComponent(selected.media_version_id)}/thumbnail?size=medium&frame=poster`} src={mediaProxyUrl(selected.media_version_id)} data-original-src={mediaContentUrl(selected.media_version_id)} onError={fallbackToOriginalVideo} onLoadedMetadata={(event) => { event.currentTarget.currentTime = selected.source_start_us / 1_000_000; }} onTimeUpdate={(event) => { const clip = timedClips.find((item) => item.shot_id === selected.shot_id); if (clip) setPlayheadUs(Math.min(clip.end_us, clip.start_us + Math.max(0, event.currentTarget.currentTime * 1_000_000 - selected.source_start_us))); }} aria-label={`${selected.shot_code} 视频预览`} /> : <p className="empty-state">当前镜头没有可播放视频。</p>}</div><div className="edit-transport"><button type="button" onClick={() => { const video = videoRef.current; if (!video) return; video.paused ? void video.play() : video.pause(); }}>播放 / 暂停</button><span>{timeLabel(playheadUs)} / {timeLabel(durationUs)}</span><label>缩放<input aria-label="时间线缩放" type="range" min="1" max="4" step="0.25" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></label><button type="button" onClick={() => setPlayheadUs(0)}>回到开头</button></div></div>
      <aside className="edit-clip-inspector" aria-label="镜头 Inspector">{selected ? <><header><div><p className="eyebrow">镜头 Inspector</p><h3>{selected.shot_code}</h3></div><span className={`status-pill ${selected.continuity_status === "CONFLICT" ? "danger" : ""}`}>{selected.continuity_status}</span></header><p title={selected.source_name ?? undefined}>{selected.source_name ?? "已采用视频"}</p><label>成片时长（秒）<input type="number" min="0.1" max="3600" step="0.1" value={(selected.duration_us / 1_000_000).toFixed(2)} onChange={(event) => updateClip({ duration_us: Math.max(100_000, Math.round(Number(event.target.value || 0.1) * 1_000_000)) })} /></label><label>源视频入点（秒）<input type="number" min="0" step="0.1" value={(selected.source_start_us / 1_000_000).toFixed(2)} onChange={(event) => updateClip({ source_start_us: Math.max(0, Math.round(Number(event.target.value || 0) * 1_000_000)) })} /></label><label>入场转场<select value={selected.transition_in} disabled={timedClips[0]?.shot_id === selected.shot_id} onChange={(event) => updateClip({ transition_in: event.target.value as EditableClip["transition_in"] })}><option value="CUT">硬切</option><option value="DISSOLVE">叠化</option><option value="FADE">淡入</option></select></label><div className="edit-reorder"><button type="button" className="secondary" onClick={() => moveClip(-1)}>前移</button><button type="button" className="secondary" onClick={() => moveClip(1)}>后移</button></div><small>调整只会进入新的 TimelineRevision；不会覆盖镜头采用、源媒体或旧冻结版本。</small></> : <p className="empty-state">选择时间线中的一个镜头进行调整。</p>}</aside></section>
    <TimelineLanes clips={timedClips} audio={data.audio_clips} subtitle={data.subtitle} durationUs={durationUs} zoom={zoom} playheadUs={playheadUs} selectedShotId={selectedShotId} includeDialogue={includeDialogue} includeMusic={includeMusic} includeSubtitles={includeSubtitles} onSelect={(shotId, startUs) => { setSelectedShotId(shotId); setPlayheadUs(startUs); }} />
    ...
    <Drawer open={historyOpen} onClose={() => setHistoryOpen(false)} title="时间线版本历史" width={560}><div className="edit-history-list">{data.history.map((item) => <article key={item.id}><div><strong>v{item.revision_no} · {item.status}</strong><span>{timeLabel(item.duration_us)} · {item.video_count} 镜 · {item.audio_count} 音频</span></div><small>{new Date(item.created_at).toLocaleString()} · {item.revision_hash.slice(0, 12)}</small></article>)}{data.history_has_more ? <p className="muted">这里只显示最近 20 个版本；完整历史通过分页 API 读取。</p> : null}</div></Drawer>
...
function TimelineLanes({ clips, audio, subtitle, durationUs, zoom, playheadUs, selectedShotId, includeDialogue, includeMusic, includeSubtitles, onSelect }: { clips: EditableClip[]; audio: EditAudioClipV2[]; subtitle: { revision_no: number; cue_count: number } | null; durationUs: number; zoom: number; playheadUs: number; selectedShotId: string | null; includeDialogue: boolean; includeMusic: boolean; includeSubtitles: boolean; onSelect: (shotId: string, startUs: number) => void }) {
  const contentWidth = Math.max(900, (durationUs / 1_000_000) * 48 * zoom);
  const position = (start: number, end: number) => ({ left: `${durationUs ? (start / durationUs) * 100 : 0}%`, width: `${durationUs ? Math.max(0.3, ((end - start) / durationUs) * 100) : 0}%` });
  const audioLane = (lane: string) => audio.filter((item) => item.lane === lane);
  return <section className="edit-timeline" aria-label="多轨编辑时间线"><div className="edit-timeline-scroll"><div className="edit-timeline-content" style={{ width: `${contentWidth}px` }}><div className="edit-ruler"><span>00:00</span><span>{timeLabel(durationUs / 2)}</span><span>{timeLabel(durationUs)}</span></div><div className="edit-playhead" style={{ left: `${durationUs ? (playheadUs / durationUs) * 100 : 0}%` }} aria-hidden="true" />
    <Lane label="V1" title="视频"><div className="edit-lane-rail">{clips.map((item) => <button type="button" key={item.shot_id} className={`edit-timeline-clip video${selectedShotId === item.shot_id ? " is-selected" : ""}`} style={position(item.start_us, item.end_us)} onClick={() => onSelect(item.shot_id, item.start_us)} title={`${item.shot_code} · ${timeLabel(item.end_us - item.start_us)}`}><b>{item.shot_code}</b><small>{item.transition_in}</small></button>)}</div></Lane>
    <AudioLane label="A1" title="对白" items={audioLane("DIALOGUE")} disabled={!includeDialogue} position={position} /><AudioLane label="A2" title="BGM" items={audioLane("BGM")} disabled={!includeMusic} position={position} /><AudioLane label="A3" title="SFX / 环境" items={[...audioLane("SFX"), ...audioLane("ENVIRONMENT"), ...audioLane("LEGACY")]} disabled={!includeMusic} position={position} />
...
function Lane({ label, title, disabled = false, children }: { label: string; title: string; disabled?: boolean; children: React.ReactNode }) { return <div className={`edit-lane${disabled ? " is-disabled" : ""}`}><header><strong>{label}</strong><span>{title}</span></header>{children}</div>; }
function AudioLane({ label, title, items, disabled, position }: { label: string; title: string; items: EditAudioClipV2[]; disabled: boolean; position: (start: number, end: number) => { left: string; width: string } }) { return <Lane label={label} title={title} disabled={disabled}><div className="edit-lane-rail">{items.map((item) => <span key={`${item.lane}:${item.id}`} className="edit-timeline-clip audio" style={position(item.start_us, item.end_us)} title={`${item.source_name} · ${item.gain_db} dB`}><b>{item.source_name}</b><small>{item.gain_db} dB</small></span>)}{items.length === 0 ? <em>暂无轨道</em> : null}</div></Lane>; }
