...
const latestRevisionId = (line: DialogueLine) => line.text_revisions.at(-1)?.id ?? "";
const currentCandidates = (line: DialogueLine) => line.candidates.filter((candidate) => candidate.dialogue_text_revision_id === latestRevisionId(line));
const hasCurrentSelection = (line: DialogueLine) => currentCandidates(line).some((candidate) => candidate.id === selectedCandidateId(line));
...
export function AudioEpisodeOverview({
  ...
  const missingTakes = lines.filter((line) => currentCandidates(line).length === 0).length;
  const missingSelection = lines.filter((line) => !hasCurrentSelection(line)).length;
  const selectedFormal = lines.filter((line) => {
    const candidateId = selectedCandidateId(line);
    return currentCandidates(line).some((candidate) => candidate.id === candidateId && candidate.candidate_kind === "FORMAL");
  }).length;
  ...
  return <>
    <div className="card-grid v2-summary-grid" aria-label="本集声音生产概况">
      ...
      <div className="status-card"><span>TTS Takes</span><strong>{lines.reduce((count, line) => count + currentCandidates(line).length, 0)}</strong><small>{missingTakes} 条无当前候选 · {missingSelection} 条未选当前音频</small></div>
