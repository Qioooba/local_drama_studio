...
export function ProjectTemplateCopyAction({ project, onCopied }: { project: Project; onCopied: (project: Project) => void }) {
  ...
  if (!open) return <button className="secondary" onClick={() => setOpen(true)}>复制为新剧模板</button>;
  return <section className="template-copy" aria-labelledby="template-copy-title">
    ...
    <div className="action-row"><button onClick={() => copy.mutate()} disabled={!code.trim() || !title.trim() || copy.isPending}>{copy.isPending ? "复制中…" : "确认复制"}</button><button className="secondary" onClick={() => setOpen(false)} disabled={copy.isPending}>取消</button></div>
  </section>;
}
