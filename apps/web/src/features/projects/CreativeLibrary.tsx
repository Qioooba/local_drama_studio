...
export function CreativeLibrary({ projectId }: { projectId: string }) {
  ...
  return <section className="creative-library" aria-labelledby="creative-library-title">
    <div className="panel-heading"><div><p className="eyebrow">FR-WRT-001 · immutable history</p><h3 id="creative-library-title">故事圣经与创作资料</h3></div><span className="status-pill">{entries.data?.items.length ?? 0} 项</span></div>
    <div className="creative-layout">
      <div className="creative-list">{entries.data?.items.map((entry) => <button key={entry.id} className={entry.id === selected ? "selected" : ""} onClick={() => setSelectedId(entry.id)}><strong>{entry.code}</strong><span>{entry.kind} · revision {entry.revision_no}</span></button>)}{entries.data?.items.length === 0 && <p className="empty-state">当前项目尚无创作资料。</p>}</div>
      <div className="creative-detail">{detail.data ? <>
        ...
        <button className="secondary" disabled={!editValid || save.isPending} onClick={() => save.mutate()}>保存新 revision</button>
        ...
        <label>回退说明<input value={restoreNote} onChange={(event) => setRestoreNote(event.target.value)} placeholder="回退会派生新的 revision" /></label><button className="secondary" disabled={!restoreValid || restore.isPending} onClick={() => restore.mutate()}>从所选旧版派生回退 revision</button>
      </> : <p className="empty-state">选择资料查看 revision。</p>}</div>
    </div>
    <details className="creative-create"><summary>新建创作资料</summary><div className="creative-create-grid"><label>类型<select value={kind} onChange={(event) => setKind(event.target.value)}><option value="">请选择</option>{kinds.map((value) => <option key={value}>{value}</option>)}</select></label><label>Code<input value={code} onChange={(event) => setCode(event.target.value)} placeholder="CHAR_MOTHER" /></label><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className="wide">初始内容 JSON<textarea value={initialContent} onChange={(event) => setInitialContent(event.target.value)} /></label><label className="wide">建立说明<input value={initialNote} onChange={(event) => setInitialNote(event.target.value)} /></label><button className="primary-action" disabled={!createValid || create.isPending} onClick={() => create.mutate()}>建立并保存 revision 1</button></div></details>
