...
type Edit = { target_duration_ms: string; shot_type: string };
...
export function StoryboardBatchWorkbench({ projectId, episodeId }: { projectId?: string; episodeId: string }) {
  ...
  const payload = (): StoryboardBatchPayload => ({
    ...
    edits: Object.entries(edits).map(([shot_id, edit]) => ({ shot_id, expected_revision: Number(byId.get(shot_id)?.revision), target_duration_ms: Number(edit.target_duration_ms), shot_type: edit.shot_type })),
    copies,
  });
  ...
  const move = (index: number, offset: number) => { const next = [...orderedIds]; const target = index + offset; if (target < 0 || target >= next.length) return; const shot = byId.get(next[index]); const anchor = next[target]; if (!shot) return; [next[index], next[target]] = [next[target], next[index]]; setOrderedIds(next); setPendingReorder(offset < 0 ? { shot_id: shot.id, before_shot_id: anchor, expected_revision: shot.revision } : { shot_id: shot.id, after_shot_id: anchor, expected_revision: shot.revision }); setSemanticPlan(null); invalidatePlan(); };
  const dropBefore = (targetId: string) => { if (!draggedId || draggedId === targetId) return; const shot = byId.get(draggedId); if (!shot) return; const next = orderedIds.filter((id) => id !== draggedId); next.splice(next.indexOf(targetId), 0, draggedId); setOrderedIds(next); setPendingReorder({ shot_id: draggedId, before_shot_id: targetId, expected_revision: shot.revision }); setSemanticPlan(null); setDraggedId(null); invalidatePlan(); };
  const update = (shot: StoryboardShot, field: keyof Edit, value: string) => { setEdits((current) => ({ ...current, [shot.id]: { target_duration_ms: current[shot.id]?.target_duration_ms ?? String(shot.target_duration_ms), shot_type: current[shot.id]?.shot_type ?? shot.shot_type, [field]: field === "target_duration_ms" ? value.replace(/^(-?)0+(?=\d)/, "$1") : value } })); invalidatePlan(); };
  ...
  const semanticCommitting = useMutation({ mutationFn: () => commitShotEdit(episodeId, semanticPayload(), String(semanticPlan?.plan_hash)), onSuccess: async () => { setPendingReorder(null); setSplits([]); setSemanticPlan(null); setSplitSource(""); await Promise.all([queryClient.invalidateQueries({ queryKey: ["storyboard", episodeId] }), queryClient.invalidateQueries({ queryKey: ["shot-edit-context", episodeId] }), queryClient.invalidateQueries({ queryKey: ["shot-group-workspace", episodeId] })]); } });
  const addSplit = () => { const shot = byId.get(splitSource); const firstDuration = Number(splitAt); if (!shot || !splitCodes.first.trim() || !splitCodes.second.trim() || firstDuration <= 0 || firstDuration >= shot.target_duration_ms) return; setSplits((current) => [...current.filter((item) => item.shot_id !== shot.id), { shot_id: shot.id, expected_revision: shot.revision, first_code: splitCodes.first.trim(), second_code: splitCodes.second.trim(), first_duration_ms: firstDuration }]); setSplitCodes({ first: "", second: "" }); setSplitAt(""); setSemanticPlan(null); };
  ...
  return <section className="subpanel storyboard-workbench" aria-labelledby="storyboard-title">
    ...
    <p className="muted">默认只呈现当前 25 镜并在工作区内部滚动；字段、重排、复制和批量命令在抽屉中处理，不会让页面随镜头数量无限增长。</p>
    ...
    <div className="storyboard-command-bar">
      ...
      <div className="storyboard-command-actions">
        ...
        <button type="button" className="secondary" onClick={() => setToolDrawer("STRUCTURE")}>重排、拆分与复制</button>
      </div>
    </div>
      ...
      {view === "TABLE" && <div className="storyboard-table" role="list" aria-label="分镜批量编辑清单">
        {visibleShots.map((shot, localIndex) => {
          ...
          return <article className="storyboard-row episode-plan-shot-row" role="listitem" draggable onDragStart={() => setDraggedId(shot.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => dropBefore(shot.id)} key={shot.id}>
            ...
            <div className="shot-identity"><strong>{shot.code}</strong><small>{shot.shot_type} · {(shot.target_duration_ms / 1000).toFixed(2)} 秒 · r{shot.revision}</small></div>
    ...
    <Drawer open={Boolean(drawerShot)} onClose={() => setShotDrawerId(null)} title={drawerShot ? `${drawerShot.code} · 镜头详情` : "镜头详情"} width={560}>
      {drawerShot && (() => {
        ...
        return <div className="storyboard-drawer-stack">
          ...
          <label>镜头类型<input aria-label={`${drawerShot.code} 类型`} value={edits[drawerShot.id]?.shot_type ?? drawerShot.shot_type} onChange={(event) => update(drawerShot, "shot_type", event.target.value)} /></label>
    ...
    <Drawer open={toolDrawer === "BATCH"} onClose={() => setToolDrawer(null)} title={`批量操作 · 已选 ${selectedIds.length} 镜`} width={620}>
      <div className="storyboard-drawer-stack">
        ...
        <label><span>批量镜头类型</span><input aria-label="批量镜头类型" value={batchShotType} onChange={(event) => setBatchShotType(event.target.value)} placeholder="例如 MEDIUM" /></label>
    ...
    <Drawer open={toolDrawer === "STRUCTURE"} onClose={() => setToolDrawer(null)} title="重排、拆分与复制" width={680} dirtyGuard={Boolean(pendingReorder || splits.length || copies.length || Object.keys(edits).length)}>
      <div className="storyboard-drawer-stack">
        <section aria-labelledby="storyboard-copy-title"><div className="section-title"><span id="storyboard-copy-title">复制镜头</span><small>创建新身份，不复用原镜头 id</small></div><div className="storyboard-copy"><label>复制来源<select aria-label="复制来源" value={copySource} onChange={(event) => { setCopySource(event.target.value); invalidatePlan(); }}><option value="">选择镜头</option>{source.map((shot) => <option key={shot.id} value={shot.id}>{shot.code}</option>)}</select></label><label>新编号<input aria-label="复制后的新编号" value={copyCode} onChange={(event) => setCopyCode(event.target.value)} placeholder="SH-024-COPY" /></label><button type="button" className="secondary" onClick={addCopy} disabled={!copySource || !copyCode.trim()}>加入复制计划</button><span className="muted">待复制 {copies.length} 项</span></div></section>
        <section aria-labelledby="storyboard-split-title"><div className="section-title"><span id="storyboard-split-title">拆分镜头</span><small>提交后保留来源谱系</small></div><div className="storyboard-copy"><label>拆分镜头<select aria-label="拆分镜头" value={splitSource} onChange={(event) => { const shot = byId.get(event.target.value); setSplitSource(event.target.value); if (shot) { setSplitAt(String(Math.floor(shot.target_duration_ms / 2))); setSplitCodes({ first: `${shot.code}-A`, second: `${shot.code}-B` }); } }}><option value="">选择镜头</option>{source.map((shot) => <option key={shot.id} value={shot.id}>{shot.code} · {shot.target_duration_ms}ms</option>)}</select></label><label>第一段时长 ms<input aria-label="第一段时长 ms" type="number" min="1" value={splitAt} onChange={(event) => setSplitAt(event.target.value)} /></label><label>第一段编号<input aria-label="第一段编号" value={splitCodes.first} onChange={(event) => setSplitCodes((current) => ({ ...current, first: event.target.value }))} /></label><label>第二段编号<input aria-label="第二段编号" value={splitCodes.second} onChange={(event) => setSplitCodes((current) => ({ ...current, second: event.target.value }))} /></label><button type="button" className="secondary" disabled={!splitSource} onClick={addSplit}>加入拆分计划</button></div></section>
