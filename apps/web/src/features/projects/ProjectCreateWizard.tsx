...
export function ProjectCreateWizard({ onCreated, profiles = [] }: { onCreated: (project: Project) => void; profiles?: Profile[] }) {
  ...
  const basePayload = useMemo<ProjectCreatePayload>(() => ({
    code: values.code.trim(), title: values.title.trim(), season_count: Number(values.seasons), episode_count: Number(values.episodes),
    target_duration_ms: Number(values.duration) * 1000, aspect_ratio: values.ratio.trim(), width: Number(values.width), height: Number(values.height),
    fps: { numerator: Number(values.fpsNum), denominator: Number(values.fpsDen) }, primary_language: values.language.trim(),
    subtitle_mode: values.subtitleMode as ProjectCreatePayload["subtitle_mode"],
    ...(values.subtitleMode !== "NONE" ? { subtitle_language: values.subtitleLanguage.trim() } : {}), allow_unconfigured_capabilities: !configureNow,
    ...(configureNow ? {
      production_plan: { code: values.planCode.trim(), title: values.planTitle.trim(), plan: { mode: "LOCAL_ONLY", aspect_ratio: values.ratio.trim(), resolution: { width: Number(values.width), height: Number(values.height) }, fps: { numerator: Number(values.fpsNum), denominator: Number(values.fpsDen) }, primary_language: values.language.trim(), subtitle_mode: values.subtitleMode } },
      profile_bindings: Object.entries(profileBindings).filter(([, version]) => version).map(([capability, profile_version_id]) => ({ capability, profile_version_id })),
      delivery_target: { code: values.targetCode.trim(), title: values.targetTitle.trim(), spec: { path_rel: values.targetPath.trim(), width: Number(values.width), height: Number(values.height), fps: { numerator: Number(values.fpsNum), denominator: Number(values.fpsDen) }, subtitle_mode: values.subtitleMode } },
    } : {}),
  }), [configureNow, profileBindings, values]);
  const storagePlan = useMutation({ mutationFn: () => planProjectCreation({ ...basePayload, allow_unconfigured_capabilities: true, production_plan: undefined, profile_bindings: [], delivery_target: undefined }), onSuccess: ({ plan }) => { if (plan.status !== "BLOCKED") setStep(4); } });
  const finalPlan = useMutation({ mutationFn: () => planProjectCreation(basePayload), onSuccess: () => setStep(6) });
  const create = useMutation({ mutationFn: () => createProject(basePayload), onSuccess: ({ project }) => onCreated(project) });
  ...
  if (!open) return <button ref={openButtonRef} type="button" className="secondary" aria-haspopup="dialog" aria-expanded={false} onClick={() => setOpen(true)}>新建项目</button>;
