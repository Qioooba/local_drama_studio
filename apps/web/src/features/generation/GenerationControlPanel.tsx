...
export function GenerationControlPanel({ timedDirections, performanceBindings, motionMasks, referenceBindings, onTimedDirectionsChange, onPerformanceBindingsChange, onMotionMasksChange, onReferenceBindingsChange, tier = "", onTierChange }: Props) {
  ...
  return <section className="generation-control-panel" aria-labelledby="generation-controls-title"><div className="section-title"><span id="generation-controls-title">导演控制与多模态输入</span><small>写入冻结 Variant 参数，不修改源媒体</small></div>
    ...
    <div className="ref2v-capability-row" aria-label="Ref2V 参考视频能力">
      ...
      {ref2va?.manifest_hint?.ref2va_unet_name && <small className="muted">manifest loader：{ref2va.manifest_hint.ref2va_unet_name}</small>}
    </div>
