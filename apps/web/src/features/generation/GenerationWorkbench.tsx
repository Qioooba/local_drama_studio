...
export function GenerationWorkbench({ projectId, profiles, candidates, h3, g6Readiness, i2vProbePlan, shots, selectedShotId, onSelectShot, onOpenProfiles, onOpenReviews, onSubmitted }: GenerationWorkbenchProps) {
  ...
  const branchMutation = useMutation({
    mutationFn: async (operation: "RESAMPLE_NEW_SEED" | "RESUBMIT_PROVIDER_RANDOM" | "PROMPT_BRANCH" | "SOURCE_IMAGE_BRANCH") => {
    ...
    <section className="panel composer-panel">
      ...
      <div className="composer-grid">
        <div className="composer-main">
          ...
          <div className="variant-strip"><strong>创作分支</strong><button className="chip active">基础生成</button><button className="chip" disabled={!submittedVariantId || branchMutation.isPending} onClick={() => branchMutation.mutate("RESAMPLE_NEW_SEED")}>同图同词 · 新 seed</button><button className="chip" disabled={!submittedVariantId || branchMutation.isPending} onClick={() => branchMutation.mutate("RESUBMIT_PROVIDER_RANDOM")}>Provider random 重提</button><button className="chip" disabled={!submittedVariantId || branchMutation.isPending} onClick={() => branchMutation.mutate("PROMPT_BRANCH")}>改 Prompt</button><button className="chip" disabled={!submittedVariantId || branchMutation.isPending} onClick={() => branchMutation.mutate("SOURCE_IMAGE_BRANCH")}>换图</button><button className="chip" disabled>首尾帧 · 待能力发布</button></div>
          {branchPlan && <section className="branch-plan-panel" aria-label="生成分支计划"><strong>分支计划 READY</strong><span>只改变：{Array.isArray(branchPlan.diff["changed_fields"]) ? branchPlan.diff["changed_fields"].map(String).join("、") : "已声明字段"}</span>{branchPlan.reproducibility?.level === "NON_REPRODUCIBLE" && <span className="blocker-text">NON_REPRODUCIBLE · 不保证重复结果</span>}<code>{branchPlan.plan_hash.slice(0, 16)}</code><button className="secondary" onClick={() => branchSubmitMutation.mutate()} disabled={branchSubmitMutation.isPending}>{branchSubmitMutation.isPending ? "提交分支中…" : "确认创建分支 Job"}</button></section>}
