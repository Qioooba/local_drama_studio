...
import { GenerationWorkbench, type GenerationStep } from "./GenerationWorkbench";
...
describe("GenerationWorkbench step workflow", () => {
  ...
  it("persists intent and prompt, preflights, and requires confirmation Dialog before submitting Variant plus Job", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "下一步：预检与确认 →" }));
    ...
    // Preflight
    fireEvent.click(screen.getByRole("button", { name: "建立意图并执行只读预检" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "确认提交任务" }));
    await waitFor(() =>
