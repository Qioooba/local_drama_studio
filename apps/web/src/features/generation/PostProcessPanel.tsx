...
import { createPostProcessRecipe, getBackgroundOperation, listPostProcessRecipes, planEnhancementRun, publishPostProcessRecipe, submitEnhancementRun, type EnhancementPlan, type EnhancementRun, type PostProcessRecipe, type ReviewInboxItem } from "../../generated/api";
...
export function PostProcessPanel({ videos }: { videos: ReviewInboxItem[] }) {
  ...
  const editorSteps = useMemo<Array<Record<string, unknown>>>(() => [
    { kind: "SCALE", width: Number(width), height: Number(height), fit: "CONTAIN", executor_ref: "builtin:ffmpeg" },
    ...optionalSteps,
    { kind: "TECHNICAL_QC", executor_ref: "builtin:ffprobe" },
    { kind: "ENCODE", codec: "H264", preset, crf: Number(crf), executor_ref: "builtin:ffmpeg" },
  ], [crf, height, optionalSteps, preset, width]);
  ...
  return <section className="panel post-process-panel" aria-labelledby="post-process-title">
    ...
    <div className="post-process-grid">
      <div className="post-process-config">
        ...
        <div className="post-process-dimensions"><label>输出分辨率<select value={resolutionValue} onChange={(event) => updateResolution(event.target.value)}>{resolutionOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label>编码质量<select value={crf} onChange={(event) => setCrf(event.target.value)}>{CRF_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label></div>
        <label htmlFor="post-preset">H.264 编码速度<select id="post-preset" value={preset} onChange={(event) => setPreset(event.target.value)}>{ENCODING_SPEED_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      ...
      <div className="post-process-runner">
        ...
        {queuedJobId && !completed && <p className="frame-feedback success" role="status"><strong>{["FAILED", "CANCELLED", "NEEDS_ATTENTION"].includes(String(operation.data?.job.state)) ? "后台任务需要处理。" : "增强任务已安全进入后台队列。"}</strong> 可以离开本页，并在<a href="/jobs">任务中心</a>查看进度、失败原因和重试入口。</p>}
