...
export function ReviewInboxPanel({
  ...
  return <section className="panel" aria-labelledby="review-inbox-title"><div className="panel-heading"><div><p className="eyebrow">G4 REVIEW INBOX</p><h3 id="review-inbox-title">媒体版本审核与选择</h3></div><span className="status-pill" aria-label={`待处理媒体版本 ${items.length} 个`}>{items.length} 个待处理</span></div>
    ...
    <div className="review-filters" role="search" aria-label="审核收件箱筛选">
      <label>媒体类型<select value={mediaFilter} onChange={(event) => { setMediaFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部</option><option value="IMAGE">IMAGE</option><option value="VIDEO">VIDEO</option><option value="AUDIO">AUDIO</option></select></label>
      <label>项目<select aria-label="项目筛选" value={projectFilter} onChange={(event) => { setProjectFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部项目</option>{projectOptions.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></label>
      <label>集<select aria-label="集筛选" value={episodeFilter} onChange={(event) => { setEpisodeFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部集</option>{episodeOptions.map(([id, label]) => <option key={id || "none"} value={id}>{label}</option>)}</select></label>
      <label>年龄<select aria-label="年龄筛选" value={ageFilter} onChange={(event) => { setAgeFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部年龄</option><option value="NEW">≤ 1 天</option><option value="AGING">1–7 天</option><option value="OLD">&gt; 7 天</option></select></label>
      <label>优先级<select aria-label="优先级筛选" value={priorityFilter} onChange={(event) => { setPriorityFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部优先级</option><option value="HIGH">HIGH / P0</option><option value="NORMAL">NORMAL / P1</option></select></label>
      <label>阻塞<select aria-label="阻塞筛选" value={blockingFilter} onChange={(event) => { setBlockingFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="ALL">全部</option><option value="BLOCKED">仅阻塞</option><option value="READY">仅可处理</option></select></label>
      <label>审核状态<select value={reviewFilter} onChange={(event) => { setReviewFilter(event.target.value); setVisibleCount(INITIAL_LIST_WINDOW); }}><option value="PENDING">待处理 / 非批准</option><option value="STALE">STALE</option><option value="ALL">全部历史</option></select></label>
      <span className="muted" role="status" aria-live="polite">显示 {filteredItems.length}/{items.length} · 读取顺序稳定，筛选不改变审核决定</span>
    </div>
    {selectedVersionId && currentSelection && <div className="selection-history-actions"><span className="muted">当前已选择 {selectionType}；选择记录可追溯，允许改选。</span><button className="secondary" onClick={() => onPromote(selectedVersionId, selectionType)} disabled={selecting}>{selecting ? "保存中…" : `再次选择为 ${selectionType}`}</button></div>}
