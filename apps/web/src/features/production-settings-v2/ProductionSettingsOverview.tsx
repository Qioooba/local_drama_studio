...
export function ProductionSettingsOverview({ projectId }: { projectId: string }) {
  ...
  const blockers: Array<{ text: string; to: string }> = [
    ...
    ...(!recipe.isPending && !recipe.data ? [{ text: "项目尚未明确绑定导演模板", to: `/projects/${projectId}/director-recipes` }] : []),
    ...(missingStages.length ? [{ text: `项目级自动质检规则尚缺：${missingStages.map(qcStageLabel).join("、")}`, to: `/projects/${projectId}/qc-policies` }] : []),
  ...
  return <div className="production-settings-overview">
    ...
    {!loading && <section className={`panel production-settings-readiness ${blockers.length ? "blocked" : "ready"}`} aria-labelledby="production-readiness-title"><div className="panel-heading"><div><p className="eyebrow">生产就绪性</p><h3 id="production-readiness-title">{blockers.length ? `${blockers.length} 项需要处理` : "项目默认配置完整"}</h3></div><span className={`status-pill ${blockers.length ? "state-blocked" : "state-active"}`}>{blockers.length ? "尚有阻塞" : "已就绪"}</span></div>{blockers.length ? <ul>{blockers.map((item) => <li key={item.text}><span>{item.text}</span><Link to={item.to}>处理</Link></li>)}</ul> : <p>配置信息均来自当前服务器；进入具体生产步骤时仍会执行实时检查。</p>}</section>}
