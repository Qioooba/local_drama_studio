...
function ProductionCanvasPanel({ episodeId }: { episodeId: string | null }) {
  ...
  return <section className="panel canvas-panel">
    ...
    <div className="canvas-node-list" aria-label="键盘节点列表">{visibleNodes.map((node) => <button key={`keyboard-${node.id}`} className={selectedNodeId === node.id ? "selected" : ""} onClick={() => setSelectedNodeId(node.id)}>{String((node.data as { label?: string }).label ?? node.id)}</button>)}</div>
