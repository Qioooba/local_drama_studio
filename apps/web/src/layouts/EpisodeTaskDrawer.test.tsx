...
import { requestJson, retryJob } from "../generated/api";
import { EpisodeTaskDrawer } from "./EpisodeTaskDrawer";

vi.mock("../generated/api", () => ({ requestJson: vi.fn(), retryJob: vi.fn() }));
...
describe("EpisodeTaskDrawer", () => {
  it("loads only the server-filtered project and episode scope and exposes recoverable failure", async () => {
    vi.mocked(requestJson).mockResolvedValue({ items: [{ id: "job-failed-1", type: "VIDEO", project_id: "p1", state: "FAILED", channel: "GPU", priority: 1, max_attempts: 3, revision: 1, stage_code: "VIDEO", last_error_detail_redacted: "显存不足" }] });
    ...
    expect(requestJson).toHaveBeenCalledWith(expect.stringContaining("project_id=p1&episode_id=e1"));
