...
- 2026-08-15 续跑后 Web 测试已升至 `14 passed`：新增生成客户端 G8 timeline/subtitle/audio/render/delivery URL/body 契约测试；TypeScript、production build、Ruff、mypy 仍全绿。
- G10 继续推进：离线 SBOM 生成器 `scripts/generate_sbom.py` 已从两个锁文件和本地元数据生成 317 个包条目，并从已安装发行包的本地许可证文件解析标准 SPDX 标识；剩余 73 项许可证为 `NOASSERTION`（主要是未安装的跨平台可选 Node 包）。只读审计新增 `SBOM_INVENTORY=PASS`，但因许可证/前置 G7/G8/G9 未完成，发布工件仍保持 DRAFT/NO-GO。
...
- 容量快照发现并清理一条 2026-08-11 遗留的 `CPU_TEST`/`g5-sse-live-1` 队列项（无 attempt、非 ComfyUI），状态从 `QUEUED` 变为 `CANCELLED`；之后队列为 0、GPU active 为 0。证据：`docs/evidence/g10/stale-job-maintenance-2026-08-14.json`。
