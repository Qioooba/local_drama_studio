...
4. **P1-9 长镜头分段**（`long_shot.py` `plan_segments` 纯函数规划器，17k+5 网格、接续锚点；`timeline.render_segmented_episode` 同参数 concat + 复用 `_register_render` 登记，render_mode=SEGMENTED_CONCAT）；`test_long_shot.py` 7 用例（含真实 ffmpeg 拼接验证）。边界：分段规划为纯函数，拼接与登记复用既有渲染路径；跨段接续锚点由规划器保证。
5. **P1-10 剪映草稿导出**（`timeline_exports.export_revision` format=jianying，draft_content.json best-effort 社区逆向格式 + 媒体随包复制 + manifest media_copy=BUNDLED；可选 subtitle_revision_id 并入文本轨；导出保持只读不写审计的不变量；TimelineExportAction"导出剪映草稿"按钮；format 参数向后兼容 OTIO/EDL 不变）；`test_jianying_export.py` 8 用例。e2e `jianying_export_windows_uat.spec.ts` PASS（真实时间线 revision + 真实 VIDEO → 面板导出 → API 验证 draft_content.json + 随包媒体），证据 `docs/evidence/g10/jianying-export-windows-uat-2026-08-17.json`。边界：draft_content.json 为社区逆向 best-effort 格式，兼容性以目标剪映版本为准。
