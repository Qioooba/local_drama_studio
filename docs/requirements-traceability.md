...
| FR-PRJ-002 v2 标准项目包 | PARTIAL / EXPORT + CONTROLLED DRY-RUN VERIFIED | `ProjectPackageService` 与 export/dry-run API/UI：结构状态、项目 payload、manifest、逐文件 SHA/size、源前后复验、partial 原子发布、确定性复用；受控目录内流式校验 zip-slip/duplicate/schema/hash/size/entry/展开/压缩比/磁盘/identity，零 DB 写入。隔离真实 Unicode 文件与恶意 ZIP 负例、三视口只读入口 UAT 通过，见 `docs/evidence/g10/project-package-uat-2026-08-15.json`。外部 staging、commit、identity 重写、断点重试、注册/缩略图仍未完成，不得标 VERIFIED |
