...
| FR-IMG-003 图片候选与比较 | PARTIAL / UAT PENDING (PASS mapping, FORMAL READONLY UAT pending) | V2 `EpisodeReviewWorkspace` 内的 `ReviewInboxPanel` 统一承载候选列表、键盘切换、A/B 缩略图比较、参考图置顶与 metadata；只使用 320px small 派生缩略图。重复的旧 `ImageCandidateGrid` 已退役。GET/HEAD 原图接口仍硬拒绝，真实用户三视口 UAT 待补，见 `fr-img-001-006-image-candidate-review-2026-08-16.json` |
...
- **时间线（UI）**：V2 `TimelineComposer` 以结构化轨道编辑创建新的不可变 TimelineRevision。
...
1. **P0-1/2 故事资产库**：`story_assets`/`shot_asset_bindings`（迁移 0040）——四类资产卡（角色/场景/道具/服装）CRUD + 乐观锁 + 归档 + canonical 参考图 + 镜头绑定/解绑 + 审计；当前 UI owner 为 V2 `AssetBiblePage`、导演台资产绑定区与连续性面板，重复的旧故事资产面板已退役；e2e `story_asset_windows_uat.spec.ts` PASS。
...
## 2026-08-17 第八轮：G11 增强批次 2（P1 七项 + P2 设计文档）

按 `docs/plan/gap-closure-development-plan.md` 完成 G11 增强批次 2（P1 七项全部落地 + P2 设计文档），4 个并行子代理开发 + 协调者集成，全部真实数据、真实页面点击验证：

1. **P1-6 交付规格预设库**（`delivery_presets.py` + ConfigurationService 扩展 + 2 路由）：7 个不可变平台预设（抖音竖屏/快手竖屏/小红书 3:4/视频号/B站横屏/通用 16:9/通用 9:16，含 width/height/fps/bitrate_kbps/max_duration_seconds/cover_aspect）；`create_delivery_target_from_preset` 复用既有校验/审计 + 独立审计 `DELIVERY_TARGET_CREATED_FROM_PRESET`；ReadinessPanels"从平台预设创建交付目标"UI；`test_delivery_presets.py` 6 用例。e2e `delivery_presets_windows_uat.spec.ts` PASS（UI 套用 DOUYIN_VERTICAL → API 验证冻结 spec 1080×1920/30fps/6000kbps/180s/封面 1080×1440），证据 `docs/evidence/g10/delivery-presets-windows-uat-2026-08-17.json`。边界：预设为不可变常量，不建表不落库；派生交付目标仍走既有 DeliveryTarget 校验/审计与 provenance 链。
2. **P1-7 生产档位**（`h3_workflows.py` PRODUCTION_TIERS 5 档 FAST/DRAFT/SCREEN/PRODUCTION/MASTER，帧数 17k+5 网格 107/107/124/175/209、默认 take 2/4/6/8/12；`build_t2va`/`build_fl2va` 可选 tier；`GET /production-tiers`；GenerationControlPanel 档位下拉 + 参数摘要，tier 冻结进 parameter_set 元数据不触白名单）；`test_production_tiers.py` 8 用例。边界：tier 仅冻结进 parameter_set 元数据，不触碰白名单/迁移；实际 H3 执行仍取决于本机 GPU 档位可用性。
3. **P1-8 Ref2V 参考视频能力位**（`h3_workflows.build_ref2va` 原生链 MiniMaxH3ReferenceToVideo，真实 ComfyUI 0.31 节点已核实；能力门禁 `ref2va_supported`/`ref2va_capability`；`GET /capabilities/ref2va`；本机 manifest 有 ref2va 模型 → supported；参考视频输入因链未验证抛 `H3_REF2VA_UNAVAILABLE`；UI 置灰 + 原因；集成时已将 MiniMaxH3ReferenceToVideo 加入 `workflows.py` TRUSTED_COMFY_BUILTINS 并经 `/object_info` 真实核实）；`test_ref2va_capability.py` 7 用例。边界：能力位只反映本机 manifest 与 ComfyUI 节点核实结果，参考视频输入在链未验证时抛错、不降级伪造。
4. **P1-9 分段合成**：`timeline.render_segmented_episode` 接收显式、已验证的 segments，执行 concat 并复用 `_register_render` 登记，`render_mode=SEGMENTED_CONCAT`。未接入生产链路的旧自动规划器已移除，系统不再宣称会自动规划长镜头分段。
5. **P1-10 剪映草稿导出**（`timeline_exports.export_revision` format=jianying，draft_content.json best-effort 社区逆向格式 + 媒体随包复制 + manifest media_copy=BUNDLED；可选 subtitle_revision_id 并入文本轨；导出保持只读不写审计的不变量；V2 `TimelineExportPanel` 提供“导出剪映草稿”；format 参数向后兼容 OTIO/EDL 不变）；`test_jianying_export.py` 8 用例。e2e `jianying_export_windows_uat.spec.ts` 覆盖冻结时间线经 V2 页面导出，证据 `docs/evidence/g10/jianying-export-windows-uat-2026-08-17.json`。边界：draft_content.json 为社区逆向 best-effort 格式，兼容性以目标剪映版本为准。
6. **P1-11 音效/BGM 轨**（`bind_audio` track_type 约束 {DIALOGUE,BGM,SFX} + 旧值兼容别名 MUSIC→BGM/ENVIRONMENT→SFX；`render_episode` 有绑定时的三步 ffmpeg 混音 concat→mix(volume/adelay/loop/fade/amix normalize=0)→mux，无绑定时单命令零改动；render_mode=MIXED_AUDIO + audio_bindings 快照；AudioTrackPanel BGM 标签）；`test_bgm_tracks.py` 3 用例（含真实 ffmpeg 混音 probe 断言）。边界：无绑定走原单命令路径零改动；有绑定混音以真实 ffmpeg probe 断言验证。
7. **P1-12 字幕样式模板**（`subtitle_styles.py` `validate_style` + SubtitleStyleTemplateService，**不建表**：复用 creative_entries kind='STYLE' 不可变 revision；`create_subtitle_revision` 可选 style 合并进 cue.style_json；ASS 输出 [V4+ Styles] 块、SRT/VTT 不变；SubtitleRevisionPanel 样式表单 + 保存/载入模板）；`test_subtitle_styles.py` 5 用例。e2e `subtitle_styles_windows_uat.spec.ts` PASS（样式表单→保存模板→ASS revision→API 验证 [V4+ Styles] + cue.style_json），证据 `docs/evidence/g10/subtitle-styles-windows-uat-2026-08-17.json`。边界：样式模板不建表，复用 creative_entries kind='STYLE' 不可变 revision；仅 ASS 输出样式块，SRT/VTT 输出不变。
8. **P2 设计文档**：`docs/plan/timeline-editor-design.md` 已交付——P2-A 可视化时间线剪辑器四阶段设计（只读轨道/拖拽/局部重剪/转场预览，数据模型全部复用现有 TimelineRevision/转场约束/字幕/音频轨，不建新表）；P2-B 封面图合成、P2-C 敏感内容预检、P2-D 多模态画布增强各半页设计与验收标准。边界：本批 P2 仅交付设计文档，不实施代码，实施排入后续批次。

门禁：API 全量 **382 passed**、Web **39 files / 126 tests**、`pnpm check` 全绿（Ruff/mypy/build/G5）、`release_audit` 保持 PASS/GO（链头 0041 未变，P1 无新迁移）。

收口：P1 全部七项满足 DoD（单测/回归/门禁全绿、e2e 真实数据页面点击 PASS、审计与 provenance 齐备）；P2 本批只交付设计文档；正式库无新增迁移（链头仍 0041）。工作区未提交改动（P1 代码 + e2e + 证据 + 本登记）由协调者统一提交。
