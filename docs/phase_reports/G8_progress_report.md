...
- 2026-08-15 续跑：生成客户端补齐 G8 时间线 revision、字幕 revision、音频绑定、渲染、delivery build/verify/withdraw、FrameAnchor、转场约束、后处理 recipe 和 enhancement run 操作，并加入 4 个 URL/body 契约测试；这些操作仍需真实输入和显式用户动作，不会自动写入生产集。
