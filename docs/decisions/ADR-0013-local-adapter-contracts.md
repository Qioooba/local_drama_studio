...
G7-03/G7-07 以静态 `AdapterContractRegistry` 统一描述 ComfyUI loopback、当前真实的 Local LLM loopback（Ollama API）、Local CLI、FFmpeg/FFprobe 四类本地能力。契约检查只验证 transport、loopback 主机、凭据和本地 executable 引用，不打开 socket、不启动进程、不加载模型、不改变数据库。
