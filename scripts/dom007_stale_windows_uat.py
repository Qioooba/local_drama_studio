...
def _serve(root: Path, port: int) -> None:
    ...
    uvicorn.run(
        create_app(_settings(root, port)),
        host="127.0.0.1",
        port=port,
        log_level="warning",
    )
...
def _video(settings: Settings) -> Path:
    if not settings.ffmpeg_path:
        raise RuntimeError(
            "LOCAL_DRAMA_FFMPEG is required for this real local-media UAT"
        )
...
def _published_profile(database: Database, settings: Settings) -> str:
    ...
    with database.transaction() as connection:
        row = connection.execute(
            "SELECT status FROM execution_profile_versions WHERE id=?",
            (profile_version_id,),
        ).fetchone()
...
def run(*, root: Path, port: int | None = None) -> dict[str, object]:
    ...
    proxy = media_service.derive_version(
        asset_id, str(imported["media_version_id"]), "PROXY"
    )
    ...
    continuity_new_formal = media_service.derive_version(
        asset_id, str(formal["id"]), "FORMAL"
    )
    continuity_timeline = TimelineService(database, settings)
    continuity_anchor = continuity_timeline.create_frame_anchor(
        str(formal["id"]), source_time_us=500_000, role_hint="LAST_FRAME"
    )
    ...
    continuity_intent = continuity_generation.create_intent(
        ...
        str(continuity_shot["id"]),
        "I2V",
        "continue continuity with stale upstream anchor",
    )
    continuity_variant = continuity_generation.create_variant(
        str(continuity_intent["id"]),
        _variant_plan(
            continuity_profile_id, str(continuity_anchor["extracted_media_version_id"])
        ),
    )
    continuity_template = next(
        item for item in reviews.templates() if item["code"] == "formal_video"
    )
    continuity_checks = [
        {"item_id": str(item["id"]), "result": "PASS"}
        for item in continuity_template["items"]
    ]
    ...
    continuity_impact = reviews.preview_approval_impact(
    ...
    revised = shot_studio_command_service(database).save_draft_revision(
        str(shot["id"]), {"subject_action": "向前走"}, freeze=True
    )
    with database.connect() as connection:
        history = connection.execute(
            "SELECT id, media_version_id, selection_type, source_revision FROM selections WHERE id=?",
            (selection["id"],),
        ).fetchone()
        continuity_anchor_row = connection.execute(
            "SELECT is_stale, stale_reason FROM frame_anchors WHERE id=?",
            (continuity_anchor["id"],),
        ).fetchone()
        ...
        continuity_variant_row = connection.execute(
            "SELECT is_stale, stale_reason FROM generation_variants WHERE id=?",
            (continuity_variant["id"],),
        ).fetchone()
    ...
    try:
        ...
        context = _http(
            base_url, f"/api/v1/subjects/MEDIA_VERSION/{formal['id']}/review-context"
        )
        loopback_stale = _http(
            ...
            payload={
                ...
                "media_version_ids": [str(formal["id"])],
            },
        ...
        stale_parent = _http(
            ...
            method="POST",
            payload={
                "operation": "RESAMPLE_NEW_SEED",
                "explicit_seed": 99,
                "branch_reason": "must reject stale parent",
            },
                ...
                "bindings": [
                    {
                        ...
                        "media_version_id": str(
                            continuity_anchor["extracted_media_version_id"]
                        ),
                        "ordinal": 0,
                    }
                ],
    ...
    reviews_payload = (
        context_payload.get("reviews")
        if isinstance(context_payload.get("reviews"), list)
        else []
    )
    review_summary = [
        {
    ...
    plan = (
        loopback_payload.get("plan")
        if isinstance(loopback_payload.get("plan"), dict)
        else {}
    )
    plan_items = plan.get("items") if isinstance(plan.get("items"), list) else []
    blockers = (
        plan_items[0].get("blockers", [])
        if plan_items and isinstance(plan_items[0], dict)
        else []
    )
    checks = [
        _check(
            "REAL_LOCAL_MP4_AND_FORMAL_QC",
            machine["status"] == "PASS",
            {"machine_status": machine["status"], "formal_version_id": formal["id"]},
            "ffmpeg-created MP4 was imported, derived, and locally QC checked",
        ),
        _check(
            "APPROVAL_AND_SELECTION_HISTORY_CREATED",
            ...
            "current formal approval enabled formal-selection preflight before the upstream change",
        ),
        _check(
            "UPSTREAM_SHOT_REVISION_CREATED",
            int(revised["revision_no"]) >= 2,
            {"shot_revision_id": revised["id"], "revision_no": revised["revision_no"]},
            "new immutable ShotRevision is the upstream change",
        ),
        _check(
            "HISTORY_RETAINED",
            history is not None
            and str(history["media_version_id"]) == str(formal["id"])
            and str(history["selection_type"]) == "FORMAL_SELECTION",
            dict(history) if history else None,
            "selection history is retained rather than deleted or rewritten",
        ),
        _check(
            "CONTINUITY_OBJECTS_STALE_PROPAGATED",
            bool(
                continuity_anchor_row
                and continuity_transition_row
                and continuity_variant_row
            )
            ...
            and continuity_transition_row["stale_reason"]
            == "approved_video_winner_changed"
            and continuity_transition_row["compatibility_status"] == "STALE"
            and int(continuity_variant_row[0]) == 1
            and continuity_variant_row["stale_reason"]
            == "approved_video_winner_changed",
            {
                "frame_anchor": continuity_anchor_row,
                "transition_constraint": continuity_transition_row,
                "generation_variant": continuity_variant_row,
            },
            "continuity winner change stales frame anchor, transition constraint, and dependent variant",
        ),
        _check(
            "CONTINUITY_WINNER_REVIEW_RECORDED",
            continuity_approval["decision"] == "APPROVED"
            and continuity_approval["id"] != approval["id"],
            continuity_approval,
            "continuity scenario includes second winner review using preview impact hash",
        ),
        _check(
            "REVIEW_MARKED_STALE",
            bool(review_summary)
            and bool(review_summary[0].get("is_stale"))
            and review_summary[0].get("stale_reason") == "shot_revision_changed",
            review_summary,
            "loopback review context exposes the exact upstream stale reason",
        ),
        _check(
            "FORMAL_SELECTION_GATE_BLOCKED",
            loopback_stale.get("status") == 200
            and plan.get("status") == "BLOCKED"
            and "LATEST_HUMAN_APPROVAL_REQUIRED" in blockers,
            {"response_status": loopback_stale.get("status"), "plan": plan},
            "the current formal-selection gate cannot reuse the now-stale approval",
        ),
        _check(
            "VARIANT_PARENT_STALE_BLOCKED",
            stale_parent.get("status") == 422
            and _error_code(stale_parent) == "VARIANT_PARENT_STALE",
            stale_parent,
            "stale winner propagation must block derive-plan with VARIANT_PARENT_STALE",
        ),
        _check(
            "FRAME_ANCHOR_STALE_INPUT_BLOCKED",
            stale_plan.get("status") == 422
            and _error_code(stale_plan) == "FRAME_ANCHOR_STALE_INPUT",
            stale_plan,
            "stale continuity frame anchor must block new generation-variant plan with FRAME_ANCHOR_STALE_INPUT",
        ),
        _check(
            "FRAME_ANCHOR_STALE_TRANSITION_BLOCKED",
            stale_transition.get("status") == 422
            and _error_code(stale_transition) == "FRAME_ANCHOR_STALE",
            stale_transition,
            "stale continuity anchor must block shot-transition creation with FRAME_ANCHOR_STALE",
        ),
        _check(
            "STALE_AUDIT_RECORDED",
            audit is not None and str(audit["action"]) == "REVIEWS_MARKED_STALE",
            {"action": audit["action"] if audit else None},
            "stale propagation has an immutable local audit event",
        ),
        _check(
            "LOOPBACK_ONLY",
            live.get("status") == 200 and base_url.startswith("http://127.0.0.1:"),
            {"endpoint": base_url, "health": live},
            "only a temporary loopback FastAPI process was contacted",
        ),
    ]
    return {
