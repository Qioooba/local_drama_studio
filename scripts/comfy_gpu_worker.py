...
def run_worker(timeout_seconds=300):
    ...
    coordinator = GpuRuntimeCoordinator(db, settings)
    service = ComfyGenerationService(db, settings, gpu_coordinator=coordinator)
    
    print(f"Checking for queued GPU_H3 jobs in database: {settings.database_path}")
    # This CLI owns the single physical device for the whole claimed attempt:
    # a bare submit would otherwise queue a heavy workflow while another
    # runtime (Ollama / llama.cpp / PyTorch) still holds VRAM.
    with coordinator.session(GpuRuntime.COMFY, owner_kind="CLI_WORKER", owner_ref="comfy-gpu-worker"):
        submission = None
        for _ in range(20):
            try:
                submission = service.submit_next("real-e2e-worker", gpu_session=False)
