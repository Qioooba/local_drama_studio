...
export async function uploadScriptDocument(projectId: string, file: File, baseUrl = ''): Promise<{ import: DocumentImport }> {
  ...
  const response = await requestJson<unknown>(`/api/v1/projects/${encodeURIComponent(projectId)}/imports:upload`, { method: 'POST', headers, body: file }, baseUrl);
  return parseDocumentImportEnvelope(response);
}
"""
    client_path.write_text(
        client_source.replace("__API_CONTRACT_VERSION__", API_CONTRACT_VERSION),
        encoding="utf-8",
    )
