...
def _http(base_url: str, path: str, *, method: str = "GET", headers: dict[str, str] | None = None) -> dict[str, object]:
    started = time.perf_counter()
    request = Request(f"{base_url.rstrip('/')}{path}", method=method, headers={"Accept": "application/json", **(headers or {})})
    try:
...
def _check(name: str, passed: bool, observed: object, detail: str) -> dict[str, object]:
    return {"code": name, "status": "PASS" if passed else "FAIL", "observed": observed, "detail": detail}
...
def run(*, root: Path, port: int | None = None, keep_root: bool = True) -> dict[str, object]:
    ...
    ranges = [
        service.bind_episode_scene_range(str(first_episodes_before[0]["id"]), str(scenes[0]["id"]), 1, 1, 8, "script:1-8"),
        service.bind_episode_scene_range(str(first_episodes_before[1]["id"]), str(scenes[1]["id"]), 1, 9, 16, "script:9-16"),
    ]
    ...
    for index, shot in enumerate(shots_before, start=1):
        revision = service.create_shot_revision(str(shot["id"]), {"subject_action": f"动作 {index}"})
    ...
    try:
        service.reorder_episode(str(target_episode["id"]), 3, expected_revision=episode_expected_revision)
    except DomainRuleError as error:
        stale_error = error.code

    shot_ids_before = [str(item["id"]) for item in shots_before]
    storyboard_payload = {"ordered_shot_ids": list(reversed(shot_ids_before)), "edits": [], "copies": []}
    storyboard_plan = service.plan_storyboard_batch(str(target_episode["id"]), storyboard_payload)
    ...
    revision_ids_after = {str(item["id"]): str(item["current_revision_id"]) for item in shots_after}
    ranges_after = service.list_episode_scene_ranges(str(first_episodes_before[0]["id"]))
    ...
    try:
        ...
        episode_read = _http(base_url, f"/api/v1/projects/seasons/{first_season['id']}/episodes")
        scene_read = _http(base_url, f"/api/v1/projects/{project_id}/scenes")
        shot_read = _http(base_url, f"/api/v1/projects/episodes/{target_episode['id']}/shots")
    ...
    checks = [
        _check("PROJECT_CREATED_IN_ISOLATED_SQLITE", bool(project_id and project["code"] == "crud_windows_uat"), {"project_id": project_id}, "real ProjectService transaction"),
        _check("SEASON_EPISODE_CRUD_READ", len(seasons_before) == 2 and len(second_episodes) == 3, {"seasons": len(seasons_before), "episodes_per_season": [len(first_episodes_before), len(second_episodes)]}, "season/episode structures are persisted and readable"),
        _check("MASTER_SCENE_AND_RANGE_BINDING", len(scenes) == 2 and len(ranges) == 2 and len(ranges_after) == 1, {"scene_ids": [item["id"] for item in scenes], "range_ids": [item["id"] for item in ranges]}, "project scene identity is referenced by episode ranges"),
        _check("EPISODE_REORDER_IDENTITY_STABLE", moved_episode["id"] == target_episode["id"] and moved_episode["code"] == target_episode["code"] and [str(item["id"]) for item in episodes_after_move] == [str(target_episode["id"]), str(first_episodes_before[0]["id"]), str(first_episodes_before[1]["id"])], {"before": [{"id": item["id"], "code": item["code"], "order": item["display_order"]} for item in first_episodes_before], "after": [{"id": item["id"], "code": item["code"], "order": item["display_order"]} for item in episodes_after_move]}, "display order changes while UUID/code stay stable"),
        _check("EPISODE_REORDER_EXPECTED_REVISION", stale_error == "REVISION_CONFLICT", {"error_code": stale_error}, "stale reorder is rejected without overwrite"),
    ...
    result: dict[str, object] = {
        ...
        "isolation": {
            ...
            "database_rel": settings.database_path.relative_to(ROOT).as_posix() if settings.database_path.is_relative_to(ROOT) else str(settings.database_path),
        ...
        "inventory": {
            "implemented_service_paths": [
                ...
                "ProjectService.create_shot/create_shot_revision/list_shots",
            ...
            "not_claimed": ["No delete UI/API for season/episode/scene/shot was added in this bounded task; FR-PRJ-004 remains PARTIAL."],
        },
        "fixture": {
            "project_id": project_id,
            "season_ids": [str(item["id"]) for item in seasons_before],
            "episode_ids": [str(item["id"]) for item in first_episodes_before + second_episodes],
...
def main() -> None:
    ...
    parser.add_argument("--root", type=Path, default=ROOT / "work" / f"project-crud-windows-{datetime.now(UTC).strftime('%Y%m%dT%H%M%SZ')}")
    parser.add_argument("--port", type=int)
    parser.add_argument("--output", type=Path, default=ROOT / "docs" / "evidence" / "g10" / "fr-prj-004-windows-uat-2026-08-16.json")
