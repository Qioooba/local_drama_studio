...
for (const viewport of viewports) test(`does not count legacy license labels at ${viewport.name}`, async ({ page }) => {
  ...
  const passed = overflow === 0 && consoleErrors.length === 0 && pageErrors.length === 0 && failedResponses.length === 0 && publicRequests.length === 0 && writes.length === 0 && originalMedia.length === 0;
  results.push({ viewport: viewport.name, status: passed ? "PASS" : "FAIL", legacy_binding_count: 4, verified_evidence_count: 0, g8_audio_check: "FAIL", horizontal_overflow_px: overflow, console_errors: consoleErrors, page_errors: pageErrors, failed_responses: failedResponses, public_requests: publicRequests, write_requests: writes, original_media_requests: originalMedia });
