...
test.afterAll(() => {
  ...
  const output = path.join(root, "docs", "evidence", "g10", "creative-library-uat-2026-08-15.json");
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(output, `${JSON.stringify({ schema_version: "fr-wrt-001-production-readonly-uat.v1", observed_at: new Date().toISOString(), mode: "LOCAL_ONLY", status: results.length === 3 && results.every((item) => item.status === "PASS") ? "PASS" : "IN_PROGRESS", project_id: projectId, production_mutated: false, screenshots_created: false, viewports: results }, null, 2)}\n`, "utf8");
});

for (const viewport of viewports) {
  test(`shows versioned creative library safely at ${viewport.name}`, async ({ page }) => {
    ...
    const panel = page.locator(".creative-library");
    await expect(panel.getByRole("heading", { name: "故事圣经与创作资料" })).toBeVisible();
    await panel.getByText("新建创作资料").click();
    await expect(panel.getByRole("button", { name: "建立并保存 revision 1" })).toBeDisabled();
    await expect(panel.getByLabel("类型").locator("option")).toHaveCount(8);
