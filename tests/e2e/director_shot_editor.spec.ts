...
for (const viewport of viewports) {
  test(`shows immutable director revision controls without mutation at ${viewport.name}`, async ({ page }) => {
    ...
    page.on("request", (request) => {
      ...
      if (url.pathname.includes("/content") || url.pathname.includes("/original") || /\.(mp4|mov|mkv|webm|wav|flac)(\?|$)/i.test(url.pathname)) originalMedia.push(request.url());
    });
    ...
    await expect(panel.getByRole("button", { name: "保存新 revision" })).toBeEnabled();
    await expect(panel.getByRole("button", { name: "标记 Production Ready" })).toBeDisabled();
